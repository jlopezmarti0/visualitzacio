...
function _selectTechnology(Inputs,prodEnergia){return(
Inputs.select(prodEnergia.map(d => d.TECNOLOGIA), {sort: true, unique: true, label: "Selecciona una tecnologia"})
)}

function _selectProvincia(Inputs,prodEnergia){return(
Inputs.select(prodEnergia.map(d => d.PROVINCIA), {sort: true, unique: true, label: "Selecciona una provincia", name_id: "prov"})
)}
...
function update_map(data) {
  ...
  d3.select("svg").
    ...
    .on('click', function(d, i) {
      var selectId = d3.selectAll(".oi-3a86ea")
        .filter(function(d,i) { 
          return this.innerText == "Selecciona una provincia";}).select("select").nodes()[0].id

      var selectedItem = d3.selectAll(".oi-3a86ea")
        .filter(function(d,i) { 
          return this.innerText == "Selecciona una provincia";})
        .selectAll("option")
      .filter( function( ){ return this.innerText == d.PROVINCIA  } )
     
      var selectedProvincia = d3.selectAll("#"+selectId)
      selectedProvincia.value = selectedItem.nodes()[0].value
      selectedProvincia.property("selectedIndex",selectedItem.nodes()[0].index);
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("viewof selectTechnology")).define("viewof selectTechnology", ["Inputs","prodEnergia"], _selectTechnology);
